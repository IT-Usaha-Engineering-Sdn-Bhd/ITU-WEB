...
function Dropdown({ label, items }: { label: string; items: { label: string; href: string }[] }) {
  ...
  return <div ref={ref} className="nav-dropdown" data-menu-open={open}
    onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false) }}
    onKeyDown={(event) => { if (event.key === 'Escape') { setOpen(false); trigger.current?.focus() } }}>
    <button ref={trigger} type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls={id}>{label}<CaretDown size={13} className={open ? 'rotate-180' : ''} /></button>
    <div className="dropdown-panel" id={id} hidden={!open}><ul>{items.map((item) => <li key={item.href}><Link href={item.href} onClick={() => setOpen(false)}>{item.label}</Link></li>)}</ul></div>
  </div>
}
export function TopNav() {
  ...
  useEffect(() => {
    ...
    return () => { document.body.style.overflow = previous; query.removeEventListener('change', resize) }
  }, [mobileOpen])
  ...
  const hidden = pathname === '/' && (stage !== 'scroll' || activeSection === null || activeSection === 'hero')
  return <header ref={header} className="site-header" data-hidden={hidden} inert={hidden} data-menu-open={mobileOpen} onKeyDown={(event) => {
    if (!mobileOpen) return
    if (event.key === 'Escape') { setMobileOpen(false); toggle.current?.focus() }
    if (event.key === 'Tab') {
      const focusable = Array.from(header.current?.querySelectorAll<HTMLElement>('a, button') ?? []).filter((element) => element.getClientRects().length)
