...
const KIND_LABEL: Record<Kind, string> = {
  portrait: 'Leadership portrait',
  milestone: 'Milestone image',
  card: 'Listing image',
  gallery: 'Gallery image',
}
const KIND_SIZES: Record<Kind, string> = {
  portrait: '(min-width: 350px) 300px, 100vw',
  milestone: '(min-width: 900px) 35vw, 80vw',
  card: '(min-width: 900px) 30vw, (min-width: 600px) 45vw, 100vw',
  gallery: '(min-width: 600px) 45vw, 100vw',
}
...
export function EntryImage({
  ...
  return (
    <div className={`entry-image entry-image--${kind}${url ? ' has-image' : ' is-placeholder'}`}>
      {url ? (
        <Image
          src={url}
