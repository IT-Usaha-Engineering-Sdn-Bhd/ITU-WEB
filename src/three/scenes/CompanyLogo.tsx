...
export function CompanyLogo({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  const onPointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    moveHandler.current = (moveEvent) => {
      dragOffset.current.y = Math.max(-1, Math.min(1, dragOffset.current.y + moveEvent.movementX * .006))
      dragOffset.current.x = Math.max(-.6, Math.min(.6, dragOffset.current.x + moveEvent.movementY * .006))
    }
    upHandler.current = () => {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const idle = reducedMotion ? 0 : Math.sin(clock.elapsedTime * .4) * .3
  ...
  return <>
    ...
    <group ref={group} position={[0, .75, 0]} onPointerDown={onPointerDown}>
