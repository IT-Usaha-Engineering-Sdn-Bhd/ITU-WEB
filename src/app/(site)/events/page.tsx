...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: data.seo?.title || 'Events', description: data.seo?.description,
    alternates: { canonical: '/events' }, openGraph: { images: image ? [{ url: image }] : undefined } }
}
...
export default async function EventsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main id="main-content" className="inner-page">
    <section className="section-shell inner-hero"><p className="eyebrow">Events</p>
    ...
    <section className="section-shell listing-section">
      <CategoryTabs items={EVENT_CATEGORIES.map((c) => ({ label: c.label, value: c.value }))} active={category} basePath="/events" paramName="category" />
      <div className="listing-grid">
        {result.docs.length === 0 && <ListingCard preview meta="Coming soon" title="More events in this category are on the way" cta="" />}
        {result.docs.map((event) => <ListingCard key={event.id} href={`/events/${event.slug}`} image={event.cover}
          meta={`${EVENT_CATEGORIES.find((c) => c.value === event.category)?.label ?? ''} · ${formatMonthYear(event.eventDate)}`}
          title={event.title} cta="View Event" />)}
      </div>
      <Pagination page={page} totalPages={result.totalPages} hrefFor={(n) => `/events?category=${category}&page=${n}`} />
    </section>
  </main>
}
