...
const STATUS_TABS = [{ label: 'Completed', value: 'completed' }, { label: 'Ongoing', value: 'ongoing' }]

export async function generateMetadata(): Promise<Metadata> {
  const data = await getProjectsPage()
  const image = mediaUrl(data.seo?.ogImage)
  return { title: data.seo?.title || 'Projects', description: data.seo?.description,
    alternates: { canonical: '/projects' }, openGraph: { images: image ? [{ url: image }] : undefined } }
}
...
export default async function ProjectsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main id="main-content" className="inner-page">
    <section className="section-shell inner-hero"><p className="eyebrow">Projects</p>
    ...
    <section className="section-shell listing-section">
      <CategoryTabs items={STATUS_TABS} active={status} basePath="/projects" paramName="status" arrows={false} />
      <div className="listing-grid">
        {result.docs.length === 0 && <ListingCard preview meta="Coming soon" title="More projects in this status are on the way" cta="" />}
        {result.docs.map((project) => <ListingCard key={project.id} href={`/projects/${project.slug}`} image={project.cover}
          ...
          title={project.title} cta="Project Details" />)}
      </div>
      <Pagination page={page} totalPages={result.totalPages} hrefFor={(n) => `/projects?status=${status}&page=${n}`} />
    </section>
  </main>
}
