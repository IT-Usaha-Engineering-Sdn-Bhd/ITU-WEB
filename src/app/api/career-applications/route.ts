...
export async function POST(request: NextRequest) {
  ...
  if (!request.headers.get('content-type')?.startsWith('multipart/form-data')) return NextResponse.json({ error: 'Expected multipart form data.' }, { status: 415 })
  ...
  try {
    ...
    for (;;) {
      ...
      if (total > MAX_BODY_BYTES) return NextResponse.json({ error: 'Request too large.' }, { status: 413 })
      chunks.push(value)
    }
  ...
  const result = validateApplication({
    ...
    vacancy: form.get('vacancy'), introduction: form.get('introduction'),
  })
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: 400 })

  const resume = form.get('resume')
  if (!(resume instanceof File)) return NextResponse.json({ error: 'Attach your résumé.' }, { status: 400 })
  ...
  if (!vacancy) return NextResponse.json({ error: 'That position is no longer open.' }, { status: 409 })
  ...
  try {
    resumeDoc = await payload.create({
      collection: 'resumes', data: {},
      file: { data: Buffer.from(resumeBytes), mimetype: 'application/pdf', name: `resume-${crypto.randomUUID()}.pdf`, size: resumeBytes.byteLength },
      overrideAccess: true,
    })
    await payload.create({
      ...
      data: {
        ...
        vacancy: vacancy.id, vacancyTitle: vacancy.title, introduction: result.data.introduction,
        resume: resumeDoc.id, status: 'new',
      },
      overrideAccess: true,
    })
    ...
    if (resumeDoc) await payload.delete({ collection: 'resumes', id: resumeDoc.id, overrideAccess: true }).catch(() => {})
    return NextResponse.json({ error: 'Your application could not be saved. Please try again.' }, { status: 503 })
  }
}
