...
const limits: Record<keyof ContactData, number> = {
  name: 120, email: 254, phone: 40, companyName: 200, companyAddress: 1000, message: 5000,
}

export function validateContact(input: unknown): { ok: true; data: ContactData } | { ok: false; error: string } {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return { ok: false, error: 'Invalid enquiry.' }
  const record = input as Record<string, unknown>
  if (Object.keys(record).some((key) => !(key in limits))) return { ok: false, error: 'Invalid enquiry.' }
