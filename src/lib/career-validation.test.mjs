...
const valid = {
  ...
  introduction: 'I would love to join.',
}
...
test('isPdf checks extension, mimetype, signature and size', () => {
  assert.equal(
    isPdf({ name: 'resume.pdf', type: 'application/pdf', size: pdfBytes.length }, pdfBytes),
    true,
  )
  assert.equal(
    isPdf({ name: 'resume.png', type: 'application/pdf', size: pdfBytes.length }, pdfBytes),
    false,
  )
  assert.equal(
    isPdf({ name: 'resume.pdf', type: 'image/png', size: pdfBytes.length }, pdfBytes),
    false,
  )
  // Disguised file: .pdf name and application/pdf mimetype, but PNG bytes.
  assert.equal(
    isPdf({ name: 'resume.pdf', type: 'application/pdf', size: pngBytes.length }, pngBytes),
    false,
  )
  assert.equal(
    isPdf({ name: 'resume.pdf', type: 'application/pdf', size: 6 * 1024 * 1024 }, pdfBytes),
    false,
  )
})
