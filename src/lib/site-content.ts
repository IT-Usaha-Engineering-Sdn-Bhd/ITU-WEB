...
function withDefaults<T extends object>(defaults: T, content: Partial<T> | null | undefined): T {
  const present = Object.fromEntries(Object.entries(content ?? {}).filter(([, value]) => value !== null && value !== undefined))
...
export const getSettings = cache(async () => {
  ...
  return {
    ...saved,
    address: saved.address || '9-1, Jalan Puteri 2/7, Bandar Puteri, 47100 Puchong, Selangor Darul Ehsan.',
